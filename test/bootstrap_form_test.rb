...
class BootstrapFormTest < ActionView::TestCase
  ...
  test "empty bootstrap form with feedback_icons option is the same as empty bootstrap form" do
    expected = %{<form accept-charset="UTF-8" action="/users" class="form-inline" id="new_user" method="post" role="form"><div style="margin:0;padding:0;display:inline"><input name="utf8" type="hidden" value="&#x2713;" /></div></form>}
    assert_equal expected, bootstrap_form_for(@user, layout: :inline, feedback_icons: true) { |f| nil }
  end

  test "bootstrap form with feedback_icons option has fedback class on all the fields" do
